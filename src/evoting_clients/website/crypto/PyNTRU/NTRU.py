...
def encrypt(pub_key, input_str):
    ...
    rand_poly = random_poly(ntru.N, int(math.sqrt(ntru.q)))
    
    output = (ntru.encrypt(Poly(input[::-1], x).set_domain(ZZ), rand_poly).all_coeffs()[::-1])

    return pickle.dumps(output), pickle.dumps(rand_poly)
...
def sign(priv_key, pub_key, input_str):
    ...
    return base64.b64encode(pickle.dumps(s.all_coeffs())).decode('utf-8')

def verify(pub_key, input_str, signed_input):
    signed_input = base64.b64decode(signed_input)
