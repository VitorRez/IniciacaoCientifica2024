...
def approve_candidate(cpf, electionid, office_name):
    try:
        response = requests.get(f"{URL}/receive_pub_key")
        ...
        if response.status_code == 200:
            ...
            response = requests.post(f"{URL}/approve_candidate", json={'message': enc_data})
            return response.json()
...
def get_election(electionid):
    try:
        ...
        if response.status_code == 200:
            ...
            response = requests.post(f"{URL}/get_election", json={'message': enc_data})
