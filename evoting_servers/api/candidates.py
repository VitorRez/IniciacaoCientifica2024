...
def create_candidate(cpf, electionid, office):
    ...
    end_election = search_end_election(electionid)
...
def approve_voter(cpf, electionid, office):
    db = connect_to_db()
    ...
    try:
        db.cursor().execute("UPDATE CANDIDATES SET APPROVED 1 WHERE CPF = %s AND ELECTIONID = %s AND OFFICE_NAME = %s;", (cpf, electionid, office,))
        db.commit()
        db.close()
...
def get_candidates(electionid):
    ...
    try:
        ...
        cursor.execute(query, (electionid, electionid, 1))
